...
const EditReview = () => {
  ...
  const dateSliced = date.slice(0, 10);
    ...
    return (
      ...
      <form className='flex justify-between items-center' onSubmit={handleReviewEdit}>
      <textarea
        className='textarea w-4/12 text-black'
        placeholder={`${review}`}
        name='editedReview'
      ></textarea>
            <button className='btn btn-sm btn-outline btn-ghost text-white'><IoCheckmarkDoneSharp className='mr-2'></IoCheckmarkDoneSharp>Save</button>
      </form>
    </div>
        </div>
    );
